...
function move(call, sense) {
  let elements,
    ...
    mv_banner = landscape ? 92 : 84.25,
    mv_cards = landscape ? 20 : 45;

  if (call === "info_banner") {
    elements = document.getElementsByClassName("offert");
    if (
      (sense === "l" && banner_count <= 0) ||
      (sense === "r" && banner_count >= 2)
    )
      return;
  ...
  if (call === "popular") {
    ...
    if (
      (sense === "l" && popular_count <= 0) ||
      (sense === "r" && popular_count >= 4)
    )
      return;
    for (const key in elements) {
      if (Object.hasOwnProperty.call(elements, key)) {
  ...
  if (call === "offer") {
    ...
    if (
      (sense === "l" && offert_count <= 0) ||
      (sense === "r" && offert_count >= 4)
    )
      return;
    for (const key in elements) {
      if (Object.hasOwnProperty.call(elements, key)) {
